...
from src.usecase.train_document import TrainLangchainDocumentUseCase, TrainLangchainDocumentRequest, TrainLangchainDocumentResponse
from src.config import LangchainTrainingServiceSA
# job for getting image from blob and create vector and store
ai_bp = func.Blueprint()


@ai_bp.function_name("fn_queue_langchain_training_job")
@ai_bp.queue_trigger(arg_name="msg", connection=None, queue_name=LangchainTrainingServiceSA.job_queue)
def fn_queue_langchain_training_job(msg: func.QueueMessage):
    try:
        logging.info(
            f"Received: {base64.b64decode(msg.get_body().decode('utf-8'))}")
        body = json.loads(base64.b64decode(msg.get_body().decode('utf-8')))
        #if "job" in body:
        dto = TrainLangchainDocumentRequest(
        ...
        usecase = TrainLangchainDocumentUseCase(dto)
        logging.info("dto passed")
        res = usecase.execute()
